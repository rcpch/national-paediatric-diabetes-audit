...
def find_affected_patients_and_visits():
    patients = Patient.objects.all().annotate(
        missing_location=Case(
            When(Q(location_bng__isnull=True) | Q(location_wgs84__isnull=True), then=True),
            output_field=BooleanField()
        ),
        missing_gp_practice=Case(
            When(Q(gp_practice_ods_code__isnull=True) | Q(gp_practice_postcode__isnull=True), then=True),
            output_field=BooleanField()
        ),
        missing_imd=Case(
    ...
    print(f"  Patients missing location: {patients.filter(missing_location__gt=0).count()}")
    print(f"  Patients missing GP practice details: {patients.filter(missing_gp_practice__gt=0).count()}")
    print(f"  Patients missing IMD quintile: {patients.filter(missing_imd__gt=0).count()}")
    print(f"  Visits missing height centiles/z-scores: {visits.filter(missing_height_data__gt=0).count()}")
    print(f"  Visits missing weight centiles/z-scores: {visits.filter(missing_weight_data__gt=0).count()}")
    print(f"  Visits missing BMI centiles/z-scores: {visits.filter(missing_bmi_data__gt=0).count()}")
    print(f"")
    ...
    patients = patients.filter(
        Q(missing_location=True) |
        Q(missing_gp_practice=True) |
        Q(missing_imd=True)
    )
...
class Command(BaseCommand):
    ...
    def update_external_patient_field(self, field_name, obj, result):
        ...
        if old_value is None and (new_value is not None and not isinstance(new_value, ValidationError)):
    ...
    def update_external_visit_fields(self, field_name, obj, result):
        requires_updating = (
            getattr(obj, f"{field_name}_centile") is None or
            getattr(obj, f"{field_name}_sds") is None
        )

        can_update = (
            getattr(result, f"{field_name}_result") is not None and
            not isinstance(getattr(result, f"{field_name}_result"), ValidationError)
        )
        ...
        if requires_updating and can_update:
            ...
            if field_name == "bmi":
                setattr(obj, "bmi", result.bmi)
                obj.save()

                print(f"\t bmi", end="", flush=True)
    ...
    def handle(self, *args, **options):
        ...
        for visit in visits:
            identifier = visit.patient.unique_reference_number or visit.patient.nhs_number
            date = visit.visit_date.strftime("%Y-%m-%d") if visit.visit_date else "unknown date"
