...
def create_oauth2_token(user, application, scopes="patient:read", pdu=None):
    ...
    token = AccessToken.objects.create(
        ...
        token=f"test-token-{user.id}-{scopes.replace(':', '-')}",
...
def test_url_resolution():
    ...
    # Test API URLs
    try:
        api_patient_list = reverse("patient-list")
        print(f"API patient-list: {api_patient_list}")
        assert api_patient_list == "/api/patients/"
    except Exception as e:
        print(f"API patient-list failed: {e}")
    
    # Test web app URLs (if you have them)
    try:
        web_patient_list = reverse("patients_list")  # Your web app URL name
    ...
    api_resolver = resolve("/api/patients/")
...
def test_no_redirects():
    ...
    response = client.get("/api/patients/", follow=False)
...
@pytest.mark.skip(reason="Skipping tests that require OAuth2 and PDU setup")
@pytest.mark.django_db
class TestPatientAPIPermissions:
    ...
    def test_patient_list_requires_patient_read_scope(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_patient_list_with_valid_read_scope(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_patient_list_filters_by_user_pdu(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_patient_list_access_by_role(self, api_client, oauth2_application, seed_users_fixture, user_role):
    ...
    def test_rcpch_audit_team_sees_all_patients(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_user_with_multiple_pdus_sees_all_their_patients(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_oauth2_token_with_pdu_profile_scoping(self, api_client, oauth2_application, seed_users_fixture):
        """Test OAuth2 tokens with PDU profile limit access to specific PDU."""
        user = NPDAUser.objects.filter(
            organisation_employers__pz_code=ALDER_HEY_PZ_CODE
        ).first()
        
        # Add user to multiple PDUs
        gosh_pdu = PaediatricDiabetesUnit.objects.get(pz_code=GOSH_PZ_CODE)
        OrganisationEmployer.objects.create(
            npda_user=user,
            paediatric_diabetes_unit=gosh_pdu,
        ...
        # Create token with PDU profile for GOSH only
        token = create_oauth2_token(user, oauth2_application, scopes="patient:read", pdu=gosh_pdu)
        
        with patch.object(token, 'pdu_profile', True), \
             patch.object(token, 'paediatric_diabetes_unit', gosh_pdu):
            
            api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {token.token}')
            
            # Mock the request to include PDU profile
            with patch('project.npda.api.viewsets.patient_viewset.hasattr') as mock_hasattr:
                mock_hasattr.return_value = True
                
                with patch.object(api_client, 'request') as mock_request:
                    mock_request.pdu_profile = True
                    mock_request.paediatric_diabetes_unit = gosh_pdu
        ...
        # Should only see GOSH patients despite user having access to both PDUs
        nhs_numbers = [patient["nhs_number"] for patient in response.data]
        gosh_nhs_numbers = [p.nhs_number for p in gosh_patients]
        ah_nhs_numbers = [p.nhs_number for p in ah_patients]
...
class TestPatientAPIResponseHeaders:
    ...
    def test_response_includes_npda_headers(self, api_client, oauth2_application, seed_users_fixture):
    ...
    @patch('project.npda.api.viewsets.patient_viewset.logger')
    def test_api_request_logging(self, mock_logger, api_client, oauth2_application, seed_users_fixture):
        ...
        # Verify logging was called (implementation depends on your logging setup)
...
class TestPatientDetailAPI:
    ...
    def test_patient_detail_by_nhs_number(self, api_client, oauth2_application, seed_users_fixture):
        ...
        api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {token.token}')
        url = reverse("patient-detail", kwargs={"pk": patient.nhs_number})
        response = api_client.get(url)
        
        assert response.status_code == HTTPStatus.OK
        assert response.data["nhs_number"] == patient.nhs_number

    def test_patient_detail_by_urn(self, api_client, oauth2_application, seed_users_fixture):
        ...
        api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {token.token}')
        url = reverse("patient-detail", kwargs={"pk": patient.unique_reference_number})
        response = api_client.get(url)
        
        assert response.status_code == HTTPStatus.OK
        assert response.data["unique_reference_number"] == patient.unique_reference_number

    def test_patient_detail_not_in_pdu_returns_404(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_patient_detail_invalid_identifier_returns_404(self, api_client, oauth2_application, seed_users_fixture):
        ...
        api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {token.token}')
        url = reverse("patient-detail", kwargs={"pk": "9999999999"})  # Non-existent NHS number
        response = api_client.get(url)
...
class TestPatientAPIEdgeCases:
    ...
    def test_user_with_no_pdu_assignment_gets_empty_queryset(self, api_client, oauth2_application):
        ...
        user = NPDAUserFactory(
            ...
            organisation_employers=[],  # No PDU assignments
        )
    ...
    def test_expired_token_returns_unauthorized(self, api_client, oauth2_application, seed_users_fixture):
    ...
    def test_token_with_multiple_scopes_works(self, api_client, oauth2_application, seed_users_fixture):
        """Test that tokens with multiple scopes including patient:read work."""
        user = NPDAUser.objects.filter(
            organisation_employers__pz_code=ALDER_HEY_PZ_CODE
        ).first()
        
        token = create_oauth2_token(user, oauth2_application, scopes="patient:read patient:write")
