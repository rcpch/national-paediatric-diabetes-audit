...
def mock_remote_calls():
    with patch(
        "project.npda.forms.patient_form.validate_postcode",
        Mock(return_value={"normalised_postcode": VALID_FIELDS["postcode"]}),
    ):
        with patch(
            "project.npda.forms.patient_form.gp_ods_code_for_postcode",
            Mock(return_value="G85023"),
        ):
            with patch(
                "project.npda.forms.patient_form.gp_details_for_ods_code",
                Mock(return_value=True),
            ):
                with patch(
                    "project.npda.models.patient.imd_for_postcode",
                    Mock(return_value=INDEX_OF_MULTIPLE_DEPRIVATION_QUINTILE),
                ):
                    yield None
...
@patch(
    "project.npda.forms.patient_form.validate_postcode",
    Mock(return_value={"normalised_postcode": "W1A 1AA"}),
)
...
@patch(
    "project.npda.forms.patient_form.validate_postcode",
    Mock(side_effect=RequestException("oopsie!")),
)
...
@pytest.mark.django_db
@patch(
    "project.npda.forms.patient_form.gp_ods_code_for_postcode", Mock(return_value=None)
)
...
def test_normalised_postcode_used_for_call_to_nhs_spine():
    ...
    with patch(
        "project.npda.forms.patient_form.validate_postcode",
        Mock(return_value={"normalised_postcode": GP_POSTCODE_WITH_SPACES}),
    ):
        with patch(
            "project.npda.forms.patient_form.gp_ods_code_for_postcode"
            ...
            assert (
                mock_gp_ods_code_for_postcode.call_args_list[0][0][0]
                == GP_POSTCODE_WITH_SPACES
            )
...
@pytest.mark.django_db
@patch(
    "project.npda.forms.patient_form.gp_details_for_ods_code", Mock(return_value=None)
)
