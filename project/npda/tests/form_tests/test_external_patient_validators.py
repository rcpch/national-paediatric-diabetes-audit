...
MOCK_GP_DETAILS_FOR_ODS_CODE = {
    "GeoLoc": {
        "Location": {"PostCode": VALID_FIELDS_WITH_GP_POSTCODE["gp_practice_postcode"]}
    }
}
...
def mock_remote_calls():
    with patch(
        ...
        AsyncMock(return_value=VALID_PATIENT_POSTCODE),
    ):
        with patch(
            "project.npda.forms.external_patient_validators.gp_details_for_ods_code",
            AsyncMock(return_value=MOCK_GP_DETAILS_FOR_ODS_CODE),
        ):
            with patch(
                "project.npda.forms.external_patient_validators.gp_ods_code_for_postcode",
                AsyncMock(return_value=VALID_FIELDS["gp_practice_ods_code"]),
            ):
                with patch(
...
async def test_validate_patient():
    ...
    assert (
        result.gp_practice_postcode
        == VALID_FIELDS_WITH_GP_POSTCODE["gp_practice_postcode"]
    )
    assert (
...
async def test_invalid_postcode():
    with patch(
        ...
        with patch(
            "project.npda.forms.external_patient_validators.lookup_terminated_postcode",
            AsyncMock(return_value=None),
        ):
            result = await validate_patient_async(
                postcode="INVALID",
...
async def test_http_error_validating_postcode():
    with patch(
        "project.npda.forms.external_patient_validators.lookup_postcode",
        AsyncMock(side_effect=HTTPError("oopsie!")),
    ):
        result = await validate_patient_async(
            postcode="INVALID",
            gp_practice_ods_code=None,
...
@patch(
    "project.npda.forms.external_patient_validators.lookup_postcode",
    AsyncMock(return_value=VALID_GP_POSTCODE),
)
...
async def test_normalised_postcode_used_for_call_to_nhs_spine():
    ...
    with patch(
        ...
        with patch(
            "project.npda.forms.external_patient_validators.gp_ods_code_for_postcode"
        ) as mock_gp_ods_code_for_postcode:
            result = await validate_patient_async(
                postcode=None,
                gp_practice_ods_code=None,
            ...
            mock_gp_ods_code_for_postcode.assert_called_once_with(
                GP_POSTCODE_WITH_SPACES, async_client
            )
    ...
    with patch(
        ...
        with pytest.raises(RuntimeError):
            await validate_patient_async(
                postcode=None,
                gp_practice_ods_code=None,
                gp_practice_postcode=VALID_FIELDS_WITH_GP_POSTCODE[
...
async def test_jersey_patient():
    ...
    with patch(
        "project.npda.forms.external_patient_validators.lookup_postcode",
        AsyncMock(side_effect=lookup_postcode_return_values),
    ):
        with patch(
            "project.npda.forms.external_patient_validators.gp_ods_code_for_postcode",
            AsyncMock(return_value="JER012"),
        ):
            with patch(
                "project.npda.forms.external_patient_validators.imd_for_postcode"
            ) as mock_imd_for_postcode:
                ...
                assert (
                    result.location_wgs84
                    == VALID_JERSEY_PATIENT_POSTCODE.location_wgs84
                )
                assert result.gp_practice_ods_code == "JER012"
                assert (
                    result.gp_practice_postcode
                    == VALID_JERSEY_GP_POSTCODE.normalised_postcode
                )
                assert result.index_of_multiple_deprivation_quintile is None
