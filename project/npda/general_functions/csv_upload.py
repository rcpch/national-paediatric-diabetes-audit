    ...
    def save_row(row):
        ...
        try:
            patient, created = Patient.objects.get_or_create(
                ...
                death_date=(row["Death Date"] if row["Death Date"] != " " else None),
        ...
        try:
            site, created = Site.objects.get_or_create(
                ...
                if row["Date of leaving service"] != " "
                else None,
                reason_leaving_service=row["Reason for leaving service"]
                if row["Reason for leaving service"] != " "
                else None,
                pdu=pdu,
                patient=patient,
            )
        ...
        try:
            obj = {
                ...
                if row["Observation Date: Hba1c Value"] != " "
                else None,
                ...
                if row["Observation Date: Urinary Albumin Level"] != " "
                else None,
                ...
                if row["Observation Date: Total Cholesterol Level"] != " "
                else None,
                "thyroid_function_date": row["Observation Date: Thyroid Function"]
                if row["Observation Date: Thyroid Function"] != " "
                else None,
                ...
                if row["Observation Date: Coeliac Disease Screening"] != " "
                else None,
                "gluten_free_diet": row[
                    "Has the patient been recommended a Gluten-free diet?"
                ]
                if row["Has the patient been recommended a Gluten-free diet?"] != " "
                else None,
                "psychological_screening_assessment_date": row[
                    "Observation Date - Psychological Screening Assessment"
                ]
                if row["Observation Date - Psychological Screening Assessment"] != " "
                else None,
                ...
                if row["Date of Level 3 carbohydrate counting education received"]
                != " "
                else None,
                ...
                if row["Date of additional appointment with dietitian"] != " "
                else None,
                ...
                if row["Date that influenza immunisation was recommended"] != " "
                else None,
                ...
                if row["Start date (Hospital Provider Spell)"] != " "
                else None,
                "hospital_discharge_date": row[
                    "Discharge date (Hospital provider spell)"
                ]
                if row["Discharge date (Hospital provider spell)"] != 0
                and row["Discharge date (Hospital provider spell)"] != " "
                else None,
