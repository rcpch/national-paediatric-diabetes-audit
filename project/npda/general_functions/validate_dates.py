def validate_date(
        ...
        if date_of_birth:
            if date_under_examination < date_of_birth:
                error = {
                    f'{date_under_examination_field_name}': [f"'{date_under_examination_label_name}' cannot be before date of birth."]
                }
                errors.append(error)
                valid = False
        ...
        if date_of_diagnosis:
            if date_under_examination < date_of_diagnosis:
                error = {
                    f'{date_under_examination_field_name}': [f"'{date_under_examination_label_name}' cannot be before date of diagnosis."]
                }
                errors.append(error)
                valid = False
        ...
        if date_of_death is not None:
            if date_under_examination > date_of_death:
                error = {
                    f'{date_under_examination_field_name}': [f"'{date_under_examination_label_name}' cannot be after date of death."]
                }
                errors.append(error)
                valid = False
