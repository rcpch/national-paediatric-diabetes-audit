...
def most_recent_modal_value_by_visit_date(values_by_date, unknown_value):
    ...
    sorted_values = sorted(
        acc.items(), key=lambda item: (item[1]["count"], item[1]["most_recent_date"])
    )
...
def merge_patient_rows_for_column(
    ...
    if model in ["Patient", "Transfer"]:
        ...
        values_by_date = (
            (row["Visit/Appointment Date"], row[heading])
            for _, row in rows.iterrows()
            if pd.notnull(row[heading])
        )
        ...
        match model_field:
            case "date_of_birth":
                rows[heading], flag_values = most_recent_modal_value_by_visit_date(
                    values_by_date, unknown_value=None
                )

            case "sex":
                rows[heading], flag_values = most_recent_modal_value_by_visit_date(
                    values_by_date, unknown_value=SEX_TYPE[-1][0]
                )
            ...
            case "diabetes_type" | "postcode" | "gp_practice_ods_code":
                rows[heading] = (
                    values_by_date[-1][1] if len(values_by_date) > 0 else None
                )  # most recent value by date
...
def merge_rows_for_patient(
    identifier_heading, rows, patient_row_index, errors_to_return
):
