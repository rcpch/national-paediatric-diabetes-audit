def gather_unique_patient_and_visit_counts(dataframe, is_jersey=False):
    ...
    if is_jersey:
        ...
        unique_patient_visits = (
            dataframe.groupby("Unique Reference Number")["Visit/Appointment Date"]
        ...
        unique_patient_visits = (
            dataframe.groupby("NHS Number")["Visit/Appointment Date"]
            .apply(lambda x: x.notnull().sum())
            .to_dict()
        )
    ...
    unique_patient_visits = {
        str(key): int(value) for key, value in unique_patient_visits.items()
    }
