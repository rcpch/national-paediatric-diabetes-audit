...
def visit_falls_within_audit_period(visit_instance, audit_start_date: date) -> bool:
    """
    Patient visits should fall within the selected audit year. However, the visit_date field can be None but other measures
    might still have been completed. This function checks if any of the measures associated with a given visit instance
    fall within the requested audit period.

    Note that postgres prefers timezone aware dates, so the audit_start_date is converted to a timezone aware date.
    """
    # convert the date to a timezone aware date
    audit_start_date = timezone.make_aware(
        date(year=audit_start_date.year, month=4, day=1)
    )
    audit_end_date = timezone.make_aware(
        date(year=audit_start_date.year + 1, month=3, day=31)
    )
    for visit_date in ALL_VISIT_DATES:
        # loops through all the potential dates in a visit instance and returns True at the first instance where the date
        # is not None and falls within the audit period
        if (
            getattr(visit_instance, visit_date[0])
            and getattr(visit_instance, visit_date) >= audit_start_date
            and getattr(visit_instance, visit_date) <= audit_end_date
        ):
            return True
...
def visit_falls_within_audit_period_Q_object(
    audit_start_date: date, prepend_query_path=None
) -> Q:
    """
