...
def get_all_field_headings(dataset_year):
    """
    Returns the complete dictionary of field headings for a given dataset year.

    Args:
        dataset_year: The dataset year (e.g., 2021, 2026)

    Returns:
        dict: Dictionary mapping field names to their official headings
    """
    if dataset_year and dataset_year >= 2026:
        return FIELD_HEADINGS_2026
    else:
        return FIELD_HEADINGS_2021


def get_field_name_from_heading(heading, dataset_year):
    """
    Reverse lookup: returns the field name for a given heading.
    Useful for CSV imports where you have the heading and need the field name.

    Args:
        heading: The official heading text
        dataset_year: The dataset year (e.g., 2021, 2026)
    ...
    Example:
        >>> get_field_name_from_heading('Stated gender', 2021)
        'sex'
        >>> get_field_name_from_heading('Sex assigned at birth', 2026)
        'sex_assigned_at_birth'
    """
    headings_dict = get_all_field_headings(dataset_year)
    # Create reverse mapping
    reverse_dict = {v: k for k, v in headings_dict.items()}
    return reverse_dict.get(heading)
