...
def retrieve_cohort_for_date(date_instance: date) -> int:
    ...
    **The audit year starts on the 1st of April and ends on the 31st of March the following year**
    cohort 1: 1st April - 30th June
    cohort 2: 1st July - 30th September
    cohort 3: 1st October - 31st December
    cohort 4: 1st January - 31st March
    """
    ...
    if date_instance < audit_start_date:
        ...
        cohort_2 = date(date_instance.year - 1, 7, 1)
        cohort_3 = date(date_instance.year - 1, 10, 1)
        cohort_4 = date(date_instance.year, 1, 1)
    else:
        # The patient was audited in the current year
        # Audit end date is on the 31st of March of every year
        cohort_2 = date(date_instance.year, 7, 1)
        cohort_3 = date(date_instance.year, 10, 1)
        cohort_4 = date(date_instance.year + 1, 1, 1)

    if date_instance < cohort_2:
        return 1
    elif date_instance < cohort_3:
        return 2
    elif date_instance < cohort_4:
        return 3
    else:
        return 4
