...
def context_from_request(request):
    ...
    pdu_choices = (
        paediatric_diabetes_units_to_populate_select_field(request.user)
        if request.user.is_authenticated
        else []
    )
    audit_period_choices = (
        get_audit_periods_user_can_see(request.user)
        if request.user.is_authenticated
        else []
    )
    ...
    for banner in banners:
        ...
        if url_matcher.match(request.path) and not banner.disabled:
            if banner.user_role_to_target and request.user:
                if (
                    request.user.role == banner.user_role_to_target
