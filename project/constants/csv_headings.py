...
def get_csv_heading_objects(dataset_year=2021):
    ...
    if dataset_year == 2026:
        return get_csv_heading_objects_2026()
...
def get_csv_heading_objects_2026():
    """
    Returns CSV heading objects for 2026 in the exact order of the published template.

    This order must match the published NPDA 2026 onwards CSV Template.
    Note: This does NOT include the unique identifier (NHS Number or URN) - those are added
    separately in get_csv_heading_objects_for_year_and_unique_identifier.
    """
    # Build a lookup dict for all 2026 headings
    headings_dict = {}
    for item in ALL_HEADINGS:
        if 2026 in item["dataset_years"]:
            headings_dict[item["model_field"]] = item

    # Return in the exact order of the published template (excluding NHS Number which is added separately)
    model_field_order = [
        "date_of_birth",  # Date of Birth
        "postcode",  # Postcode of usual address
        "sex",  # Sex assigned at birth
        "ethnicity",  # Ethnic Category
        "adhd_asd_status",  # Has the patient had a diagnosis of ADHD or ASD?
    ...
    return tuple(headings_dict[field] for field in model_field_order)
