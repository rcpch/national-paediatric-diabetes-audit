...
!!! warning "IMPORTANT"
    ...
    <https://localhost:8007> is served from the `zensical serve` command and **has auto-reload**, so is much more convenient for local development, you can instantly see changes made to the Markdown files in `documentation/docs`.
...
On top of the basic features of Markdown, Zensical adds all the nice website appearance and many additional features for making beautiful documentation sites.
...
`zensical serve` starts up a development server which will auto-reload after changes to the source files, and will serve the documentation on [`localhost:8007`](http://localhost:8007).
